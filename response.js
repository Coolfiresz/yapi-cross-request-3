...
function getConnection() {
    if (!isExtensionContextValid()) {
        console.error('Extension context invalidated. Cannot create connection.');
        return null;
    }
    if (!connect) {
...
function injectJs(path) {
    if (!isExtensionContextValid()) {
        console.error('Extension context invalidated. Cannot inject script.');
        return;
    }
...
function sendAjaxByBack(id, req, successFn, errorFn) {
    // 首先检查扩展上下文是否有效
    if (!isExtensionContextValid()) {
        console.error('Extension context invalidated. Cannot send request.');
    ...
    function sendMessage(message) {
        ...
        if (!connection) {
            ...
            errorFn({
                ...
                body: 'Failed to establish connection with extension.\n无法建立与扩展的连接。'
            });
            return;
        }
        ...
        try {
            ...
            console.error('Failed to send message, reconnecting...', error);
            ...
            if (!connection) {
                errorFn({
                    status: 0,
                    statusText: 'Connection Error',
                    body: 'Failed to reconnect. Please reload the page.\n重连失败，请刷新页面。'
                });
                return;
            }
            ...
            try {
                ...
                errorFn({
                    status: 0,
                    statusText: 'Connection Error',
                    body: 'Connection error: ' + retryError.message
                });
            }
        }
    }
    ...
    if (req.files) {
      ...
      for (var name in req.files) {
        let fileTransfer = new Promise(function (resolve, reject){
          var reqFile = document.getElementById(req.files[name]);
          if (reqFile !== null) {
            var files = reqFile.files;
            let file = files[0]
...
function run() {
    // 检查扩展上下文是否有效
    if (!isExtensionContextValid()) {
        console.error('Extension context invalidated. Stopping request processing.');
        return;
    }
...
var findDom = setInterval(function () {
    try {
        // 检查扩展上下文是否有效
        if (!isExtensionContextValid()) {
            console.error('Extension context invalidated. Stopping initialization.');
        ...
        if (yRequestDom) {
            ...
            var runInterval = setInterval(function () {
                // 检查扩展上下文是否有效
                if (!isExtensionContextValid()) {
                    console.error('Extension context invalidated. Stopping request processing loop.');
